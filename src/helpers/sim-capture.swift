...
var gTargetUDID: String? = nil

struct CaptureConfig {
    var maxMirrorWidth: CGFloat = 540
    var maxFps: Double = 30
    var jpegQuality: CGFloat = 0.42
}

var gCaptureConfig = CaptureConfig()

func parseArgs(_ argv: [String]) {
    var i = 1
    while i < argv.count {
        let arg = argv[i]
        if arg == "--max-width", i + 1 < argv.count {
            gCaptureConfig.maxMirrorWidth = CGFloat(Double(argv[i + 1]) ?? 540)
            i += 2
            continue
        }
        if arg == "--max-fps", i + 1 < argv.count {
            gCaptureConfig.maxFps = Double(argv[i + 1]) ?? 30
            i += 2
            continue
        }
        if arg == "--quality", i + 1 < argv.count {
            gCaptureConfig.jpegQuality = CGFloat(Double(argv[i + 1]) ?? 0.42)
            i += 2
            continue
        }
        if arg.hasPrefix("--") {
        ...
        if gTargetUDID == nil {
            gTargetUDID = arg
            eprint("[sim-capture] target udid=\(arg)")
        }
        i += 1
    }
    eprint("[sim-capture] config width=\(Int(gCaptureConfig.maxMirrorWidth)) maxFps=\(Int(gCaptureConfig.maxFps)) quality=\(gCaptureConfig.jpegQuality)")
}
...
func bootedDevice(_ deviceSet: AnyObject) -> NSObject? {
    ...
    for d in devices {
        ...
        if let target = gTargetUDID {
            let udid = (dev.value(forKey: "UDID") as? NSUUID)?.uuidString ?? ""
            if udid != target { continue }
        }
        return dev as? NSObject
    }
    return nil
}
...
final class Stream {
    ...
    func handle(surface: IOSurface) {
        let cfg = gCaptureConfig
        let minEmitInterval = cfg.maxFps > 0 ? 1.0 / cfg.maxFps : 1.0 / 30.0
        ...
        var ci = CIImage(ioSurface: surface)
        let srcW = CGFloat(IOSurfaceGetWidth(surface))
        if srcW > cfg.maxMirrorWidth {
            let s = cfg.maxMirrorWidth / srcW
            ci = ci.transformed(by: CGAffineTransform(scaleX: s, y: s))
        }

        let opts: [CIImageRepresentationOption: Any] = [
            CIImageRepresentationOption(rawValue: kCGImageDestinationLossyCompressionQuality as String): cfg.jpegQuality
        ]
        ...
        frameCount += 1
        let reportNow = Date()
        let dt = reportNow.timeIntervalSince(lastReportTime)
        if dt >= 5 {
...
parseArgs(Array(CommandLine.arguments))
